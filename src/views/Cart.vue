        ...
        <table v-if="goodsDitailed.length>0" class="spa-table">
            ...
            <transition-group leave-active-class="prod-leave" teg="tbody">
                <tr class="spa-table-row" v-for="good in goodsDitailed" :key="good.id">
                    ...
                    <td class="spa-amount">
                        <div class="spa-div">
                            <button class="btn btn-warning" @click="decCart({id:good.id})" :disabled="good.inProcessing" :class="good.inProcessing ? 'blok-btn': ''">-1</button>
                            <input type="text" class="spa-input" :value="good.cnt" @change="chengCart({id:good.id,e:$event})" />
                            <button class="btn btn-success" @click="incCart({id:good.id})" :disabled="good.inProcessing" :class="clearCartBlok ? 'blok-btn': ''">+1</button>
                        </div>
                    </td>
                </tr>
            </transition-group>
