    ...
    <div v-if="id && item" class="spa-cont-good">
        ...
        <div class="spa-cont-buttons">
            <transition enter-active-class="btn-enter" leave-active-class="btn-leave" mode="out-in">
                <button v-if="checkInCart(id)" class="btn btn-info btn-block persp" @click="decCart({id:id})" key="remove" :disabled="item.inProcessing" :class="item.inProcessing ? 'blok-btn': ''">Remove from cart</button>
                <button v-else class="btn btn-info btn-block persp" @click="addCart({id:id})" key="add" :disabled="item.inProcessing" :class="item.inProcessing ? 'blok-btn': ''">Add to cart</button>
            </transition>
        </div>
        <h1>{{item.title}}</h1>
